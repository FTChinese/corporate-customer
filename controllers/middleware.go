...
func RequireLoggedIn(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		claims, err := admin.ParsePassportClaims(ss)
		if err != nil {
...
func getPassportClaims(c echo.Context) admin.PassportClaims {
	return c.Get("claims").(admin.PassportClaims)
}

func CheckInviteeClaims(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		authHeader := c.Request().Header.Get("Authorization")
		ss, err := ParseBearer(authHeader)
		if err != nil {
		...
		claims, err := admin.ParseInviteeClaims(ss)
		if err != nil {
			log.Printf("Error parsing JWT %v", err)
			return render.NewUnauthorized(err.Error())
		}
...
func getInviteeClaims(c echo.Context) admin.InviteeClaims {
	return c.Get("claims").(admin.InviteeClaims)
}
